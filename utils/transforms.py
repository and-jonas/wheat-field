...
def set_input_transform_options(
        predict_size,
        predict_scale,
        head="deeplabv3plus",
        train_size=700,
    ...
    # adjust predict size to something that is compatible with the model
    predict_size = (int(predict_size[0] * predict_scale), int(predict_size[1] * predict_scale))
    predict_size_adj = (predict_size[0] + (divisor - predict_size[0] % divisor), predict_size[1] + (divisor - predict_size[1] % divisor))
    ...
    class SemSegInputTransform(InputTransform):
        ...
        def predict_per_sample_transform(self) -> Callable:
            return ApplyToKeys(DataKeys.INPUT,
                               K.geometry.Resize(predict_size_adj, interpolation="nearest"),
                               K.augmentation.Normalize(mean=self.mean, std=self.std),
                               )
